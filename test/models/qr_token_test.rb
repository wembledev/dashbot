...
class QrTokenTest < ActiveSupport::TestCase
  ...
  test "find_valid returns unexpired unused token" do
    qr = QrToken.create_for_session("sess")
    assert_equal qr, QrToken.find_valid(qr.token)
  end

  test "find_valid returns nil for used token" do
    qr = QrToken.create_for_session("sess")
    qr.update!(used: true)
    assert_nil QrToken.find_valid(qr.token)
  end

  test "find_valid returns nil for expired token" do
    qr = QrToken.create_for_session("sess")
